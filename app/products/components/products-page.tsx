...
import { Edit, Search, Package, Plus, Power, ChevronLeft, ChevronRight, Loader2, Eye, Clock, XCircle, ChevronDown, ChevronUp } from 'lucide-react'
...
// DropdownMenu is no longer needed for main product row, but kept for potential future use or if variants need it.
// import {
//   DropdownMenu,
//   DropdownMenuContent,
//   DropdownMenuItem,
...
unit: string
category: string
status: "active" | "out_of_stock" // Modified: Removed low_stock
isActive: boolean // Main product active status
description?: string
stock?: number
createdAt?: string
updatedAt?: string
variants?: { name: string; price: number; isActive: boolean }[] // Modified: Added isActive
}
...
const initialProducts: Product[] = [
{
  ...
  variants: [
    { name: "หัว", price: 150, isActive: true },
    { name: "ท้อง", price: 550, isActive: true },
    { name: "หาง", price: 300, isActive: true },
    { name: "ทั้งตัว", price: 450, isActive: true },
  ],
},
{
  id: "2",
...
},
{
  ...
  unit: "กิโลกรัม",
  category: "หอย",
  status: "active", // Modified from low_stock
  isActive: true,
...
},
{
  ...
  variants: [
    { name: "เล็ก", price: 180, isActive: true },
    { name: "กลาง", price: 220, isActive: true },
    { name: "ใหญ่", price: 280, isActive: true },
  ],
},
{
  id: "7",
...
const productStatusOptions = ["ทั้งหมด", "พร้อมจำหน่าย", "หมด"] // Modified: Removed สต็อกต่ำ
const statusMap = {
active: { label: "พร้อมจำหน่าย", variant: "default" as const },
out_of_stock: { label: "หมด", variant: "destructive" as const }, // Modified: Removed low_stock
}
                ...
                <Badge
                  className={`
                    ${product.status === "active"
                      ...
                      : "bg-red-100 text-red-800 border-red-200" // Modified: Removed low_stock condition
                    }
                  `}
                >
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                  ...
                  <span className="font-medium text-gray-900">
                    {product.updatedAt ? new Date(product.updatedAt).toLocaleDateString("th-TH") : "ไม่ระระบุ"}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${product.isActive ? "bg-green-500" : "bg-red-500"}`}
                    ></div>
                    <span className="font-medium text-gray-900">
...
const [updatingProductId, setUpdatingProductId] = React.useState<string | null>(null) // Can be productId or productId-variantName
...
const filteredProducts = React.useMemo(() => {
  return products.filter((product) => {
    ...
    const matchesStatus =
      ...
      (statusFilter === "หมด" && product.status === "out_of_stock") // Modified: Removed low_stock condition
...
// Toggle expand/collapse for product variants
...
// Handle toggling active status for main product or variant
const handleToggleActive = async (productId: string, variantName?: string) => {
  const idToUpdate = variantName ? `${productId}-${variantName}` : productId
  setUpdatingProductId(idToUpdate)
  ...
  try {
      ...
      prevProducts.map((product) => {
        if (product.id === productId) {
          if (variantName && product.variants) {
            // Toggle specific variant isActive
            return {
              ...product,
              variants: product.variants.map((variant) =>
                variant.name === variantName ? { ...variant, isActive: !variant.isActive } : variant,
              ),
            }
          } else {
            // Toggle main product isActive and cascade to variants
            const newMainProductIsActive = !product.isActive
            if (product.variants) {
              return {
                ...product,
                isActive: newMainProductIsActive,
                variants: product.variants.map((variant) => ({
                  ...variant,
                  isActive: newMainProductIsActive,
                })),
            ...
            return { ...product, isActive: newMainProductIsActive }
          }
        }
        return product
      }),
    ...
    const itemToToggleName = variantName
      ? `${product?.name} (${variantName})`
      : product?.name

    const currentIsActive = variantName
      ? product?.variants?.find((v) => v.name === variantName)?.isActive
      : product?.isActive

    toast({
      title: "อัปเดตสำเร็จ",
      description: `${itemToToggleName} ได้รับการ${currentIsActive ? "ปิด" : "เปิด"}ใช้งานแล้ว`,
    })
  } catch (error) {
    toast({
...
// Handle toggling active status for all variants of a product
const toggleAllVariantsActive = async (productId: string, status: boolean) => {
  setUpdatingProductId(`all-variants-${productId}`) // Unique ID for batch update
  try {
    ...
    setProducts((prevProducts) =>
      prevProducts.map((product) => {
        if (product.id === productId && product.variants) {
          return {
            ...product,
            variants: product.variants.map((variant) => ({ ...variant, isActive: status })),
          }
        }
        return product
      }),
    ...
    toast({
      title: "อัปเดตสำเร็จ",
      description: `ตัวเลือกทั้งหมดของ ${product?.name} ได้รับการ${status ? "เปิด" : "ปิด"}ใช้งานแล้ว`,
    })
  } catch (error) {
    toast({
      title: "เกิดข้อผิดพลาด",
      description: "ไม่สามารถอัปเดตสถานะตัวเลือกได้",
      variant: "destructive",
      })
  } finally {
    setUpdatingProductId(null)
...
const handleCardFilter = (filterType: string) => {
  ...
  if (activeCardFilter === filterType) {
    ...
    } else if (filterType === "out_of_stock") { // Modified: Removed low_stock
      setStatusFilter("หมด")
    }
  }
}
...
const stats = React.useMemo(() => {
  ...
  const outOfStockProducts = products.filter((p) => p.status === "out_of_stock").length // Modified: Removed lowStockProducts
  return { totalProducts, activeProducts, outOfStockProducts } // Modified: Removed lowStockProducts
}, [products])
...
if (isLoading) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3"> {/* Modified: Changed to md:grid-cols-3 */}
        {Array.from({ length: 3 }).map((_, i) => ( // Modified: Changed to length 3
          <Card key={i}>
        ...
        <div className="text-sm text-blue-800">
          ...
          {activeCardFilter === "all"
            ...
            : activeCardFilter === "active"
              ? "สินค้าพร้อมจำหน่าย"
              : "สินค้าหมด"} {/* Modified: Removed low_stock text */}
        </div>
        <Button
          variant="outline"
          size="sm"
      ...
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>
              แสดง {startIndex + 1}-{Math.min(endIndex, filteredProducts.length)}{" "}
              จาก {filteredProducts.length} รายการ (ทั้งหมด {stats.totalProducts} รายการ)
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
      ...
      <CardContent>
        <div className="rounded-md border">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-[250px] text-left">สินค้า</TableHead><TableHead className="w-[150px] text-center">ราคาขาย</TableHead><TableHead className="w-[70px] text-center">หน่วย</TableHead><TableHead className="w-[90px] text-center">หมวดหมู่</TableHead><TableHead className="w-[180px] text-right pr-4">การดำเนินการ</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                currentProducts.map((product) => (
                  <React.Fragment key={product.id}>
                    <TableRow>
                      ...
                      </TableCell><TableCell className="text-xs text-center">{product.unit}</TableCell><TableCell className="text-center">
                        <Badge variant="outline" className="text-xs border-blue-200 bg-blue-50 text-blue-700">
                          {product.category}
                        </Badge>
                      </TableCell><TableCell className="text-right pr-4">
                        <div className="flex justify-end gap-2 items-center">
                          ...
                          <AlertDialog>
                              ...
                              <div className="flex items-center gap-1">
                                {updatingProductId === product.id ? (
                                  ...
                                  <Switch
                                    ...
                                    onCheckedChange={() => {}} // onCheckedChange will be handled by AlertDialogAction
                            ...
                            <AlertDialogContent className="border-0 rounded-3xl shadow-2xl">
                              <AlertDialogHeader>
                                <AlertDialogTitle>
                                  ยืนยันการ{product.isActive ? "ปิด" : "เปิด"}ใช้งานสินค้า
                                </AlertDialogTitle>
                                <AlertDialogDescription>
                                  คุณแน่ใจหรือไม่ที่จะ{product.isActive ? "ปิด" : "เปิด"}ใช้งานสินค้า "
                                  {product.name}"
                                  {product.variants && product.variants.length > 0 && (
                                    <span className="mt-2 text-sm text-muted-foreground block"> {/* Changed div to span and added block display */}
                                      (การดำเนินการนี้จะ{product.isActive ? "ปิด" : "เปิด"}ใช้งานตัวเลือกย่อยทั้งหมดของสินค้านี้ด้วย)
                                    </span>
                                  )}
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                    ...
                    {product.variants && expandedRows[product.id] &&
                      product.variants.map((variant) => (
                        <TableRow key={`${product.id}-${variant.name}`} className="bg-gray-50 border-l-4 border-blue-200">
                          <TableCell className="pl-12 py-2 flex items-center gap-3">
                            <span className="font-normal text-sm text-gray-700">{variant.name}</span>
                          </TableCell><TableCell className="text-center">
                            <span className="font-medium text-xs">฿{variant.price.toLocaleString()}</span>
                          </TableCell><TableCell className="text-xs text-center">{product.unit}</TableCell><TableCell className="text-center">
                            <Badge variant="outline" className="text-xs border-blue-200 bg-blue-50 text-blue-700">
                              {product.category}
                            </Badge>
                            ...
                            <div className="flex justify-end items-center">
                              <AlertDialog>
                                <AlertDialogTrigger asChild>
                                  <div className="flex items-center gap-1">
                                    {updatingProductId === `${product.id}-${variant.name}` ? (
                                      <Loader2 className="h-4 w-4 animate-spin" />
                                    ) : (
                                      <Switch
                                        ...
                                        onCheckedChange={() => {}} // onCheckedChange will be handled by AlertDialogAction
                                        className="scale-75 cursor-pointer data-[state=checked]:bg-green-500"
                                        disabled={updatingProductId === `${product.id}-${variant.name}`}
                                      />
                                    )}
                                  </div>
                                </AlertDialogTrigger>
                                <AlertDialogContent className="border-0 rounded-3xl shadow-2xl">
                                  <AlertDialogHeader>
                                    <AlertDialogTitle>
                                      ยืนยันการ{variant.isActive ? "ปิด" : "เปิด"}ใช้งานตัวเลือก
                                    </AlertDialogTitle>
                                  <AlertDialogDescription>
                                      คุณแน่ใจหรือไม่ที่จะ{variant.isActive ? "ปิด" : "เปิด"}ใช้งานตัวเลือก "
                                      {variant.name}" ของสินค้า "{product.name}"
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
                                    <AlertDialogCancel>ยกเลิก</AlertDialogCancel>
                                    <AlertDialogAction
                                      onClick={() => handleToggleActive(product.id, variant.name)}
                                      className={
                                        variant.isActive
                                    ...
                                    >
                                      {variant.isActive ? "ปิดใช้งาน" : "เปิดใช้งาน"}
                                    </AlertDialogAction>
                                  </AlertDialogFooter>
                                </AlertDialogContent>
                              </AlertDialog>
