...
},
{
  ...
  phone: "02-123-4567",
  type: "credit",
  creditInfo: {// Added creditInfo for existing credit customers
    creditType: "amount",
    creditLimit: 50000,
    creditBills: 0,
    creditDays: 30,
...
paymentMethod: string // Updated to string for bank names
printed: boolean
...
const [paymentMethod, setPaymentMethod] = React.useState<string>("") // New state for payment method
...
const handleUpdateOrder = () => { // Renamed from handleCreateOrder
  const updatedOrderData: OrderPreviewData = { // Renamed from newOrderData
    id: `ORD-${Math.floor(Math.random() * 10000).toString().padStart(4, "0")}`, // Mock ID
    ...
    paymentMethod: paymentMethod || (customerType === "cash" ? "เงินสด" : "ยังไม่ระบุ"), // Use selected or default
    printed: false,
  ...
  console.log("Updating order:", updatedOrderData) // Changed log message
  setPreviewOrderData(updatedOrderData) // Set data for preview
  setShowCreateConfirm(false) // Still using showCreateConfirm for the dialog state
  ...
  <div className="space-y-6">
    ...
    <div className="flex items-center gap-4">
      ...
      <div>
        ...
        <p className="text-muted-foreground">แก้ไขข้อมูลคำสั่งซื้อสำหรับลูกค้า</p> {/* Updated description */}
      </div>
    </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  {isExistingCreditCustomerSelected && (
                    <div className="space-y-2 mt-4 p-3 bg-blue-100 rounded-lg border border-blue-200">
                      <h4 className="font-semibold text-sm text-blue-800">ข้อมูลเครดิตค้างชำงระ:</h4>
        ...
        <Card className="border-0 shadow-sm rounded-2xl bg-white">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="flex flex-col gap-2 mt-2">
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="ธนาคารกสิกรไทย" id="kasikorn" />
                  <Label htmlFor="kasikorn" className="flex items-center gap-2">
                    <Banknote className="h-4 w-4" /> ธนาคารกสิกรไทย
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="ธนาคารไทยพานิช" id="scb" />
                  <Label htmlFor="scb" className="flex items-center gap-2">
                    <Banknote className="h-4 w-4" /> ธนาคารไทยพานิช
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="ธนาคารกรุงเทพ" id="bangkok" />
                  <Label htmlFor="bangkok" className="flex items-center gap-2">
                    <Banknote className="h-4 w-4" /> ธนาคารกรุงเทพ
                  </Label>
                </div>
              </RadioGroup>
            </div>
        ...
        <Card className="sticky top-4 border-0 shadow-sm rounded-2xl bg-white">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {discountAmount > 0 && (
                <div className="flex justify-between text-red-600">
                  <span>
                    ส่วนลดทั้งบิล ({discountType === "percentage" ? `${calculatedDiscountValue}%` : `฿${calculatedDiscountValue}`}
                    ):
                  </span>
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              {paymentMethod && <p>ช่องทางชำระเงิน: {paymentMethod}</p>} {/* Display payment method */}
            ...
            <div className="space-y-2">
              ...
              <AlertDialog open={showCreateConfirm} onOpenChange={setShowCreateConfirm}>
                ...
                <AlertDialogContent className="border-0 rounded-3xl shadow-2xl">
                  <AlertDialogHeader>
                    <AlertDialogTitle>ยืนยันการแก้ไขคำสั่งซื้อ</AlertDialogTitle>
                    <AlertDialogDescription>ต้องการยืนยันการแก้ไขคำสั่งซื้อใช่หรือไม่?</AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>ยกเลิก</AlertDialogCancel>
                    <AlertDialogAction onClick={handleUpdateOrder}>ยืนยันการแก้ไขคำสั่งซื้อ</AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
              </AlertDialog>
            </div>
    ...
    <Dialog open={showOrderCreatedSuccess} onOpenChange={setShowOrderCreatedSuccess}>
      <DialogContent className="max-w-md border-0 rounded-3xl shadow-2xl">
        <DialogHeader>
          <DialogTitle className="text-center">คำสั่งซื้อถูกแก้ไขเรียบร้อยแล้ว!</DialogTitle>
        </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Badge variant="outline" className="ml-2 bg-purple-50 text-purple-700 border-purple-200">
                      {previewOrderData.paymentMethod}
                    </Badge>
                  </div>
                </div>
              </CardContent>
